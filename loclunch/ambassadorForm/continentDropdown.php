<?php
/**
** [continent_auto] and [continent_auto*]
**/

/* form_tag handler */

add_action( 'wpcf7_init', 'tc_csca_add_form_tag_continent' );

function tc_csca_add_form_tag_continent() {
	wpcf7_add_form_tag(
		array( 'continent_auto', 'continent_auto*'),
		'tc_csca_continent_auto_form_tag_handler', array( 'name-attr' => true ) );
}

function tc_csca_continent_auto_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
    // var_dump($tag);
	...
	$class = wpcf7_form_controls_class( $tag->type, 'wpcf7-select continent_auto' );
	$atts = array();
...
$html='<span class="wpcf7-form-control-wrap continent_auto '.$tag->name.'">';
$html.='<select '.$atts.' >';
$html.='<option value="0">Select Continent</option>';
global $wpdb;
$tbl='continent';	
$continents = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM ".$wpdb->base_prefix."%1s", $tbl) );
foreach($continents as $continent)
{
$html.="<option value='".esc_html($continent->name)."' data-id='".$continent->id."'>".esc_html($continent->name)."</option>";
}
$html.='</select></span>';
return $html;
}
...
add_filter( 'wpcf7_validate_continent_auto', 'tc_csca_continent_validation_filter', 10, 2 );
add_filter( 'wpcf7_validate_continent_auto*', 'tc_csca_continent_validation_filter', 10, 2 );

function tc_csca_continent_validation_filter( $result, $tag ) {
	$type = $tag->type;
	$name = $tag->name;
	$value = sanitize_text_field($_POST[$name]);
	if ( $tag->is_required() && '0' == $value ) {
		$result->invalidate( $tag, 'Please Select Continent.' );
	}
...
add_action( 'wpcf7_admin_init', 'tc_csca_add_tag_generator_continent_auto', 20 );

function tc_csca_add_tag_generator_continent_auto() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'continent_auto', __( 'continent drop-down', 'tc_csca' ),
		'tc_csca_tag_generator_continent' );
	
}

function tc_csca_tag_generator_continent( $contact_form, $args = '' ) {
	$args = wp_parse_args( $args, array() );
	$type = 'continent_auto';

	$description = __( "Generate a form-tag for a continent dorp list with flags icon text input field.", 'tc_csca' );

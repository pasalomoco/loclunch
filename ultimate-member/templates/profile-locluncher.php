<?php /* Template: locluncher PROFILE */ ?>
...
<div class="um <?php echo esc_attr( $this->get_class( $mode ) ); ?> um-<?php echo esc_attr( $form_id ); ?> um-role-<?php echo esc_attr( um_user( 'role' ) ); ?> ">
	...
	<div class="um-form" data-mode="<?php echo esc_attr( $mode ) ?>">
		...
		echo do_shortcode( '[ambassadorProfile]' );
		/**
		 * UM hook
		 *
		 * @type filter
		...
		if ( um_is_on_edit_profile() || UM()->user()->preview ) {
			...
			if ( $menu_enabled || ! empty( $tabs[ $nav ]['hidden'] ) ) { ?>
				...
				<div class="um-profile-body <?php echo esc_attr( $nav . ' ' . $nav . '-' . $subnav ); ?>">
					...
					if  ( ! isset($_GET["profiletab"]) && in_array( 'ambassador', (array) $user->roles )) {
						echo do_shortcode( ' [event_dashboard] ' );
					} 
					if(isset($_GET["profiletab"]) && $_GET["profiletab"] == "events"){
						header ("Location: /user/$user->user_login/");
						echo "<script> window.location.href = '/user/$user->user_login/';  </script>";
						exit();
					}
